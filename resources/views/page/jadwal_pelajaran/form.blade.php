...
<div class="container-fluid">
    <div class="card">
        ...
        <form class="form-horizontal" action="{{ route($url, $jadwal_pelajaran->id_jadwal_pelajaran ?? null) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($jadwal_pelajaran))
            @method('put')
            @endif
            <div class="card-body">
                <h4 class="card-title text-center">Jadwal Pelajaran</h4>
                <hr>
                <div class="form-group">
                    <label>Hari</label>
                    <select name="id_hari" id="id_hari"
                        class="form-control @error('id_hari') {{ 'is-invalid' }} @enderror">
                    ...
                    @if(isset($jadwal_pelajaran))
                    <script>
                        document.getElementById('id_hari').value =
                            '<?php echo $jadwal_pelajaran->id_hari ?>'
                    </script>
                    @endif
                    @error('id_hari')
                    ...
                    <div class="form-group  col-md-6">
                        ...
                        <script>
                            document.getElementById('jam_awal').value =
                                '<?php echo $jadwal_pelajaran->id_jam_ajar   ?>'
                        </script>
                        @endif
                        @error('jam_awal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group  col-md-6">
                        ...
                        <script>
                            document.getElementById('id_kelas').value =
                                '<?php echo $jadwal_pelajaran->id_kelas ?>'
                        </script>
                        @endif
                        @error('id_kelas')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <script>
                        document.getElementById('nama_peg').value =
                            '<?php echo $jadwal_pelajaran->id_pegawai ?>'
                    </script>
                    @endif
                    @error('nama_peg')
                    <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <script>
                        document.getElementById('nama_pelajaran').value =
                            '<?php echo $jadwal_pelajaran->id_pelajaran ?>'
                    </script>
                    @endif
...
@if(isset($jadwal_pelajaran))
    <script>
        $(document).ready(function () {
            var jam_awal = '<?php echo $jadwal_pelajaran->id_jam_ajar ?>';
...
@if(isset($jadwal_pelajaran))
    <script>
        $(document).ready(function () {
            var id_kelas = '<?php echo $jadwal_pelajaran->id_kelas ?>';
            axios.get("{{ url('kelas/get') }}/" + id_kelas)
                .then(function(res) {
                    var hasil = res.data
                    console.log(hasil);
